...
aria2c --allow-overwrite=true --auto-file-renaming=true --bt-enable-lpd=true --bt-detach-seed-only=true \
       ...
       --max-connection-per-server=10 --max-concurrent-downloads=5 --max-file-not-found=0 --max-tries=20 \
       --min-split-size=10M --optimize-concurrent-downloads=false --peer-id-prefix=-qB4390- --reuse-uri=true \
